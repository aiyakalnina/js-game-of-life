...
function toggleCell(findClickedCell) {
  ...
  console.log(currentCellStates[selectedCellId]);
}
...
function handleMouseDrag() {
  ...
  window.addEventListener('mousemove', e => {
    ...
    console.log(isSelectingOnDrag, xClickPosition, yClickPosition);
  });
  ...
  document.addEventListener('keydown', event => {
    if (event.keyCode === 39) {
      ...
      currentCellStates.forEach((currentCell, index) => {
        ...
        // if (cell.isLive && (neighbourCount === 2 || neighbourCount === 3)) {
        //   // keep live
        // }
      });

      console.log(currentCellStates[1]);
      console.log(cells[1]);
